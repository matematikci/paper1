...
\begin{abstract}
\end{abstract}
\section{Setting Training}
We designed deep neural networks in four levels. We trained classifiers using features extracted from pretrained networks. We applied the following steps to prevent overfitting for each layer:

\subsection{Data Collection}

\subsection{Neural Network Creation}

\subsection{Configuration of Neural Network Inputs and Outputs}

Configuration involves arranging the network so that it is compatible with the problem we desired to solve, as defined by the data. 
...
Configuration and training require that the network be provided with the data. This topic shows how to format the data for presentation to the network. It also explains network configuration and the two forms of network training: incremental
...
Distilled neural networks already have been used based on ensemble methods for several types of data. What about applying distillation on specific methods with their own ensembled variations? Applying such an approach on fMRI data gives us a clear answer that it brings higher accuracy. We distilled the knowledge through the ensembled LSTM variations and compared to Hinton's approach and other 1,2,3.... The accuracy scala shows that our approach outperforms other references.

\section{Future Work}

Distilled LSTMs needs to be proved on other types of data such as Image, Audio, Text and other types of time series whether they have an overall superiority. Other types of deep neural network architectures could be benchmarked with their ensembled variations. 
